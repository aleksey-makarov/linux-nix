...
#      };
    ...
    virtualisation.mountHostNixStore =
      mkOption {
        type = types.bool;
        default = !cfg.useNixStoreImage;
        defaultText = literalExpression "!cfg.useNixStoreImage";
        description = lib.mdDoc ''
...
#        ''
    ...
    # boot.loader.supportsInitrdSecrets = mkVMOverride false;
    ...
    virtualisation.qemu.options = mkMerge [
      ...
      (let
        ...
        directBoot-initrd-default = "${config.system.build.initialRamdisk}/${config.system.boot.loader.initrdFile}";
      in [
        "-kernel \${NIXPKGS_QEMU_KERNEL_${sanitizeShellIdent config.system.name}:-${config.system.build.toplevel}/kernel}"
        "-initrd ${directBoot-initrd-default}"
...
#      ])
    ...
    in lib.mkMerge [
      ...
      {
        # FIXME
        "/" = lib.mkIf true (if cfg.diskImage == null then {
          device = "tmpfs";
          fsType = "tmpfs";
        } else {
